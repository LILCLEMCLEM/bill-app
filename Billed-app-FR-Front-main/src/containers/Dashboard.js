...
export default class {
  ...
  handleShowTickets(e, bills, index) {
    ...
    if (this.counter[index] === 1) {
      $(`#arrow-icon${this.index + 1}`).css({ transform: 'rotate(0deg)'})
      $(`#status-bills-container${this.index + 1}`)
        .html(cards(filteredBills(bills, getStatus(this.index + 1))))
      this.counter[index] = 1
    } else {
      $(`#arrow-icon${this.index+1}`).css({ transform: 'rotate(90deg)'})
      $(`#status-bills-container${this.index + 1}`)
        .html("")
      this.counter[index] = 0
    }
